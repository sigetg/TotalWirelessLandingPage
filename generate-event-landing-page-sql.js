...
// Parse the CSV file
const csvContent = fs.readFileSync('data/EVENT_LANDING_PAGE_FORMATTED.csv', 'utf-8');
...
async function generateSQL() {
  console.log('🚀 Generating Event Landing Page SQL with geocoding...\n');
  
  const sqlStatements = [];
  
  sqlStatements.push("-- Insert Event Landing Page events with geocoding");
  sqlStatements.push("-- These use ON CONFLICT to prevent duplicates");
  ...
  // Write to file
  fs.writeFileSync('event-landing-page-complete.sql', fullSQL);
  
  console.log(`\n✅ SQL file generated: event-landing-page-complete.sql`);
  ...
  console.log('  railway connect Postgres < event-landing-page-complete.sql');
}
